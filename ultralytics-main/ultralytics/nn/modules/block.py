...
class LDConv(nn.Module):
    def __init__(self, ch, group=16,reduction=16) -> None:
        ...
        self.AgvPool = nn.AdaptiveAvgPool2d(1)
    ...
    def forward(self, x):
        ...
        y = self.AgvPool(x).view(b, c)
        y = self.L2(self.L1(y)).view(b, c, 1, 1)
        att=rearrange(x * y.expand_as(x), 'bs ch (s1 h) (s2 w) -> bs ch h w (s1 s2)', s1=2, s2=2)
...
class DCNv3_Bottleneck(Bottleneck):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv2 = DCNv3_CBS(c_, c2, k[1])

class DCNv3_C2f(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(DCNv3_Bottleneck(self.c, self.c, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
########################################################DCNv3_C2f##########################################
