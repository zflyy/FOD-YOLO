...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, GREHead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3,VoVGSCSP,DCNv3_C2f):
        ...
        elif m in (Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, GREHead):
            ...
            c2 = ch[f[-1]]
        elif m in (LDConv,BiLevelRoutingAttention_nchw,DCNv3_C2f):
            c2 = ch[f]
            args = [c2, *args]
        else:
            c2 = ch[f]
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, GREHead)):
                return "detect"
